...
if(isset($_POST['submit']))
{
	...
	//check Duplicate
	$check = check_duplicate($connection,$tblname,"city_name = '$city_name' and $tblpkey <> '$keyvalue'");
	if($check > 0)
	{
	...
	else
	{
		if($keyvalue == 0)
		{
			...
			$form_data = array('city_name'=>$city_name,'state_id'=>$state_id,'pin_code'=>$pin_code,'ipaddress'=>$ipaddress,'createdate'=>$createdate);
			  dbRowInsert($connection,$connection,$tblname, $form_data);
			  $action=1;
			  $process = "insert";
		}
		else
             ...
             <?php include("../include/alerts.php"); ?>
                    ...
                    <form class="stdform stdform2" method="post" action="">
                            ...
                            <p>
                                ...
                                <select name="state_id" id="state_id" class="input-xxlarge chzn-select">
                                        ...
                                        <?php
                                        $sql = mysqli_query($connection,"select state_id,count_id,state_name from master_state order by state_id desc");
                                        while($row = mysqli_fetch_assoc($sql))
                                         {  
										   $count_id = $row['count_id'];
                                             
											 $count_name = $cmn->getvalfield($connection,"master_country","count_name","count_id = '$count_id' ");
            	...
            	<table class="table table-bordered" id="dyntable">
                    ...
                    <tbody>
                               ...
                               <?php
											$slno=1;
											$sql_get = mysqli_query($connection,"select * from master_city where 1=1 order by city_id desc");
											while($row_get = mysqli_fetch_assoc($sql_get))
											{
											  $state_name = $cmn->getvalfield($connection,"master_state","state_name","state_id = '$row_get[state_id]' "); 
											   $count_id = $cmn->getvalfield($connection,"master_state","count_id","state_id = '$row_get[state_id]' ");
												$count_name = $cmn->getvalfield($connection,"master_country","count_name","count_id = '$count_id' ");
