  ...
  var $aligns;
	...
	function Header()
	{
		...
		$this->Cell(192,5,"Date : ".date('d-m-Y'),0,1,'R');
		 ...
		 $this->Cell(90,6,'Income Name',1,0,'L',0);
		 $this->Cell(60,6,'Income Date',1,0,'L',0);
...
$pdf->SetTitle($title1);
$title2 = "Income Details";
$pdf->SetTitle($title2);
$pdf->AliasNbPages();
$pdf->AddPage('P','A4');
$slno = 1; 	 
$sql_get = mysqli_query($connection,"select * from other_income where 1=1 order by incom_date desc");
	...
	$expen_name=$cmn->getvalfield($connection,"master_expence","expense_name","expencetypeid='$row_get[expencetypeid]'");
		...
		$pdf->Row(array($slno++,$expen_name,$cmn->dateformatindia($row_get['incom_date']),$row_get['amount']));
	 }
$pdf->Output();
?> 
