  ...
  var $aligns;
	...
	function Header()
	{
		...
		$this->Cell(192,5,"Date : ".date('d-m-Y'),0,1,'R');
		 ...
		 $this->Cell(20,6,'Sale No',1,0,'L',0);
		 $this->Cell(25,6,'Sale Date',1,0,'L',0);
		 $this->Cell(65,6,'Customer Name',1,0,'L',0);
		 $this->Cell(45,6,'Sale Type',1,0,'L',0);
		 $this->Cell(35,6,'Amount',1,1,'L',0);
		 $this->SetWidths(array(10,20,25,65,45,35));
...
$sql_get = mysqli_query($connection,"Select * from saleentry $crit  order by saledate desc,saleid desc");
	while($row_get = mysqli_fetch_assoc($sql_get))
	{
		...
		$supparty_name = $cmn->getvalfield($connection,"m_supplier_party","supparty_name","suppartyid='$suppartyid'");
										...
										$total =$row_get['totalsale']-$return_amt;
		...
		$pdf->Row(array($slno++,$row_get['billno'],$cmn->dateformatindia($row_get['saledate']),$supparty_name,$row_get['billtype'],number_format(round($total),2)));
		$nettotal +=$total;
    }
	 ...
	 $pdf->SetX(5);
		 ...
		 $pdf->Cell(35,6,number_format(round($nettotal),2),1,0,'R',0);
