...
$productid = 0;
  ...
  $pageheading = ucfirst($prodname)." - ".ucfirst($catname)." Sale Return Detail"; 
...
if($todate !='' && $fromdate !='')
{
	...
	$crit .=" and sale_retdate between '$fromdate' and '$todate' ";
...
$sql_list = mysqli_query($connection,"Select * from  salereturn where productid = '$productid' $crit order by sale_retdate desc");
if($sql_list)
{
   while($row_list = mysqli_fetch_array($sql_list))
   {
	  
	$sale_retdate  = $row_list['sale_retdate'];
	...
	$pdf->Cell(40, 5,$cmn->dateformatindia($sale_retdate), 1, 0, "C", 0);
