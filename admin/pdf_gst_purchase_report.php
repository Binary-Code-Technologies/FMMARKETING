...
class PDF_MC_Table extends FPDF
{
  ...
  var $aligns;
	...
	function Header()
	{
		...
		//$this->Cell(55,5,"Supplier : ".$supparty_name,0,1,'L');
		$this->Ln(1);
		$this->SetX(5);
		...
		$this->Cell(37,6,'Supplier Name',1,0,'L',0);
...
$nettaxamt =0;
  ...
  $sql_get = mysqli_query($connection,"Select * from purchaseentry $crit order by purchasedate desc,purchaseid desc");
	while($row_get = mysqli_fetch_assoc($sql_get))
	{
		...
		$pdf->Row(array($slno++,$row_get['billno'],$cmn->dateformatindia($row_get['purchasedate']),$supparty_name,number_format($billamount,2),number_format($cgst,2),number_format($sgst,2),number_format($igst,2),number_format($cgst+$sgst+$igst,2)));
