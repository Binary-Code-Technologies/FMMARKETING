...
class PDF_MC_Table extends FPDF
{
  ...
  var $aligns;
	...
	function Header()
	{
		global $title1,$title2,$supparty_name,$address,$address2,$cust_address,$mobile,$comp_mobile,$diseasename,$billno,$saledate,$gsttinno,$comp_name,$remark,$gender,$age;
		 // courier 25
	...
	/*	$this->Ln(4);
		...
		$this->Cell(55);
		$this->SetFont('courier','b',9);
		$this->Cell(10,0,$address2,0,1,'C');
		
		$this->Ln(6);
		$this->SetY(6);
		$this->SetX(2);
		$this->SetFont('courier','b',8);
		$this->Cell(140,0,'Mobile No.: '.$comp_mobile,0,0,'R');
	    $this->Ln(3);
	    $this->Cell(82);
		 // courier bold 15
		$this->SetY(20);
	...
	//	$this->Cell(144,0,'',T,0,0,'C');
	   ...
	   // $this->Cell(40,5,$supparty_name,0,0,'L');
		...
		$this->SetX(6);
		$this->SetFont('courier','b',9);
	    $this->Cell(15,5,"Gender  :",0,0,'R');
		$this->SetFont('courier','b',9);
	   // $this->Cell(33,5,$mobile,0,0,'C');
		$this->Cell(28,5,$gender,0,0,'L');
		
		$this->SetFont('courier','b',9);
	    $this->Cell(70,5,"Age :",0,0,'R');
		$this->SetFont('courier','b',9);
	   $this->Cell(25,5,$age,0,1,'L');
	   
		$this->SetX(6);
		$this->SetFont('courier','b',9);
	    $this->Cell(10,5,"Contact :",0,0,'C');
		$this->SetFont('courier','b',9);
	   // $this->Cell(33,5,$mobile,0,0,'C');
		$this->Cell(33,5,$mobile,0,0,'C');
	   ...
	   $this->SetX(6);
		...
		$this->SetFont('courier','b',9);
	   // $this->Cell(33,5,$mobile,0,0,'C');
		$this->Cell(33,5,$cust_address,0,0,'C');
		
		$this->SetFont('courier','b',9);
	    $this->Cell(70,5,"Disease Type :",0,0,'R');
		$this->SetFont('courier','b',9);
	...
	$this->SetY(-22);
		...
		$this->SetX(2);
		 $this->Cell(60,5, "Term and Conditions ",0,'1','L',0);
		 $this->SetX(2);
		 $this->SetFont('Arial','',8); 
		$this->SetTextColor(0,0,0);
		$this->MultiCell(80,5,$term_cond,0,'L');
...
$pdf->Cell(30,6,'Product Name',1,0,'L',1);
$pdf->Cell(15,6,'UNIT',1,0,'C',1);
$pdf->Cell(18,6,'Batch No',1,0,'L',1);
$pdf->Cell(18,6,'Exp.Date',1,0,'L',1);
$pdf->Cell(10,6,'Qty',1,0,'C',1);

$pdf->Cell(11,6,'Rate',1,0,'R',1);
$pdf->Cell(13,6,'Disc(%)',1,0,'R',1);

$pdf->Cell(20,6,'AMOUNT',1,1,'R',1);

$pdf->SetX(2);
$pdf->SetWidths(array(9,30,15,18,18,10,'11',13,20));
$pdf->SetAligns(array("C","L","C","L","L","C","R","R","R"));
			...
			while($row_get = mysqli_fetch_assoc($sql_get))
{
	...
	$pdf->SetFont('Arial','',8);
	$pdf->SetTextColor(0,0,0);
	$pdf->Row(array($slno,$prodname,$unit_name,$row_get['batchno'],$row_get['expiry_date'],$row_get['qty'],number_format($rate,2),$row_get['disc_per'],number_format($amount,2)));
...
$pdf->SetFont('arial','b',10);
$pdf->SetTextColor(0,0,0);
$pdf->Cell(124,5,'Consultancy Fees :',0,0,'R',0);
$pdf->Cell(20,5,number_format($consultancy_fees,2),'0',1,'R',0);

$pdf->SetX(2);
$pdf->SetFont('arial','b',9);
$pdf->SetTextColor(0,0,0);
$pdf->Cell(124,5,'NET TOTAL :',0,0,'R',0);
$pdf->Cell(20,5,number_format(round($total+$taxamt+$igstamt+$consultancy_fees),2),'0',1,'R',0);
 $pdf->Ln(2);
 $pdf->SetX(5);
 ...
 $pdf->Cell(31,5,'AMOUNT IN RUPEES :',0,'0','L',0);
   ...
   $pdf->SetFont('Arial','b',8);
  $pdf->SetTextColor(0,0,0);
 $pdf->Cell(101,5,ucfirst(convert_number_to_words(round($total+$taxamt+$igstamt+$consultancy_fees)))." Only ",0,1,'L',0); 
