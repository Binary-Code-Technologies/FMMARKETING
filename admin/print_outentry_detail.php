...
$sql_list = mysqli_query($connection,"SELECT A.*, saledate,branch_id,billno, saletype, billtype, pcatid FROM saleentry_detail as A left join saleentry as B on A.saleid=B.saleid left join m_product as C on A.productid = C.productid where pcatid = '$pcatid' $crit order by saleid");
$total=0;
if($sql_list)
{
   while($row_list = mysqli_fetch_array($sql_list))
   {
		...
		$saledate=$row_list['saledate']; 
		$billno=$row_list['billno']; 
		$saletype=$row_list['saletype']; 
		$qty=$row_list['qty']; 
		$rate=$row_list['rate']; 
	...
	$pdf->Cell(20, 5,$cmn->dateformatindia($saledate), 1, 0, "C", 0);
	$pdf->Cell(22, 5,$billno , 1, 0, "R", 0);
	$pdf->Cell(18, 5,$billtypename, 1, 0, "R", 0);
	$pdf->Cell(18, 5,$saletype, 1, 0, "R", 0);
